            ...
            {isLoading ? (
              ...
              orders.map((order, index) => (
                ...
                >
                  ...
                  <TableCell className="text-sm max-w-[200px] truncate">{order.title}</TableCell>
                  <TableCell className="text-sm text-muted-foreground">{order.item_number || "—"}</TableCell>
                  <TableCell className="text-right text-sm">{order.quantity}</TableCell>
                  <TableCell className="text-right text-sm">{formatCurrency(order.unit_price)}</TableCell>
                  <TableCell className="text-right text-sm">
                    {order.discount ? `${order.discount}%` : "—"}
                  </TableCell>
                  <TableCell className="text-right text-sm font-medium">{formatCurrency(order.total_price)}</TableCell>
                  <TableCell className="text-right text-sm text-muted-foreground">{formatCurrency(order.cost_price)}</TableCell>
                </motion.tr>
              ))
            ) : (
              <TableRow>
