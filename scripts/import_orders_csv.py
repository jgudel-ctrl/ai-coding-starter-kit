...
def load_csv_data(filepath):
    print(f"📖 Lese CSV: {filepath}")
    data = []
    with open(filepath, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f, delimiter=';')
        for row in reader:
            data.append({
                'kundennummer': row['kundennummer'].strip(),
                'datum': row['datum'],
...
def check_partner_mapping(conn, kundennummern):
    ...
    """)
    db_kunden = {str(row[0]): {'id': row[1], 'name': row[2]} for row in cur.fetchall()}
    found = 0
    not_found = []
...
def main():
    filepath = '/tmp/orders_import.csv'
