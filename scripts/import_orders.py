#!/usr/bin/env python3
"""
Import-Script für Aufträge aus Excel-Datei
...
Usage:
    python3 import_orders.py /path/to/abholung_export.xlsx
"""
...
def import_orders(conn, data, db_kunden, admin_user_id):
    """Importiert die Aufträge in die Datenbank"""
    print("\n📥 Starte Import...")
...
def verify_import(conn):
    ...
    # Beispiele
    cur.execute("""
        SELECT o.id, p.company_name, o.status, o.geplantes_abholdatum, o.tatsaechliches_abholdatum
        FROM tms.orders o
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 import_orders.py <excel-file>")
        sys.exit(1)
    ...
    try:
        ...
        print(f"\n🎉 Import abgeschlossen!")
        print(f"   {inserted} Aufträge erfolgreich importiert")
